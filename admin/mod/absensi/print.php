...
echo'
    <section class="container_box">
      ...
      $query_present_status = "SELECT present_name FROM present_status WHERE present_id IN (SELECT present_id FROM leave_requests WHERE leave_type = '{$row_absen['leave_type']}')";
      $result_present_status = $connection->query($query_present_status);
      $row_present_status = $result_present_status->fetch_assoc();

      $present_name = $row_present_status['present_name'];
      ...
      if (isset($_GET['from']) || isset($_GET['to'])) {
        $month = $_GET['from'];
        $year = $_GET['to'];
        $filter = "employee_id='$id' AND MONTH(leave_date)='$month' AND YEAR(leave_date)='$year'";
    } else {
        $filter = "employee_id='$id' AND MONTH(leave_date) ='$month'";
    }
    
    $query_leave = "SELECT leave_type, COUNT(*) as count FROM leave_requests WHERE $filter GROUP BY leave_type";
    $result_leave = $connection->query($query_leave);
    
    $leave_stats = array();
    while ($row_leave = $result_leave->fetch_assoc()) {
        $leave_type = $row_leave['leave_type'];
        $count = $row_leave['count'];
        $leave_stats[$leave_type] = $count;
    }
    ...
    // Display leave stats
    foreach ($leave_stats as $leave_type => $count) {
        echo '<p>' . $leave_type . ' : <span class="label label-primary">' . $count . '</span></p>';
    }
...
case 'allpdf':
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      $mpdf->WriteHTML($html);
      $mpdf->Output('Data_Absensi_' . $row['employees_name'] . '_' . $month . '_' . $year . '.pdf', 'D');
    }
  } else {
    echo '<div class="alert alert-danger">Tidak ada data yang ditemukan!</div>';
  }
  break;
